...
export interface AiTailorResult {
  ...
  error?: string;
  // Closed-Loop Evaluator-Optimizer verification fields
  verifiedAtsScore?: number;
  baselineAtsScore?: number;
  scoreGain?: number;
  verificationReport?: AtsScoreReport;
  iterationCount?: number;
  loopStatus?: 'converged_target' | 'single_pass_optimal' | 'fallback_rule_based';
}
  ...
  try {
    if (typeof chrome !== 'undefined' && chrome.storage?.local) {
      return new Promise((resolve) => {
        chrome.storage.local.get(['resumehack_ai_settings'], (result: any) => {
...
export class AiTailorService {
  ...
  public async tailorBulletsWithAi(
    ...
    try {
      ...
      if (!result.usedAi || result.diffs.length === 0) {
        return {
          ...result,
          archetype,
          baselineAtsScore: atsReport.overallScore,
          verifiedAtsScore: atsReport.overallScore,
          scoreGain: 0,
          loopStatus: 'fallback_rule_based',
        };
      }

      // ── CLOSED-LOOP EVALUATOR-OPTIMIZER ────────────────────────────────────
      // 1. Simulate the mutated resume by applying the generated diffs to original bullets
      let mutatedResumeText = bullets.map(b => b.originalText).join('\n');
      for (const diff of result.diffs) {
        if (diff.originalText && mutatedResumeText.includes(diff.originalText)) {
          mutatedResumeText = mutatedResumeText.replace(diff.originalText, diff.tailoredText);
        } else {
          mutatedResumeText += `\n${diff.tailoredText}`;
        }
      }

      // 2. Score the simulated document using the deterministic 7-factor AtsScorerService
      const atsScorer = new AtsScorerService();
      let verifiedReport = atsScorer.analyze(mutatedResumeText, jobDescription);
      let verifiedScore = verifiedReport.overallScore;
      let iterationCount = 1;
      let activeDiffs = [...result.diffs];

      // 3. Auto-Refinement Pass: If verified score is below target (90) and critical keywords remain
      const targetScore = 90;
      const stillMissing = verifiedReport.keywords
        .filter(k => !k.foundInResume && (k.importance === 'Critical' || k.importance === 'Recommended'))
        .map(k => k.keyword);

      if (verifiedScore < targetScore && stillMissing.length > 0 && iterationCount < 2) {
        try {
          iterationCount++;
          ...
          const refinementResult = await this.routeAndExecute(refinementPrompt, bullets, settings);
          if (refinementResult.usedAi && refinementResult.diffs.length > 0) {
            // Merge refined diffs into activeDiffs
            for (const newDiff of refinementResult.diffs) {
              const existingIdx = activeDiffs.findIndex(d => d.id === newDiff.id || d.originalText === newDiff.originalText);
              if (existingIdx >= 0) {
                activeDiffs[existingIdx] = newDiff;
              } else {
                activeDiffs.push(newDiff);
              }
            }

            // Re-simulate score after refinement
            let refinedText = bullets.map(b => b.originalText).join('\n');
            for (const diff of activeDiffs) {
              if (diff.originalText && refinedText.includes(diff.originalText)) {
                refinedText = refinedText.replace(diff.originalText, diff.tailoredText);
              }
            }
            verifiedReport = atsScorer.analyze(refinedText, jobDescription);
            verifiedScore = verifiedReport.overallScore;
          }
        } catch (refineErr) {
          console.warn('[AiTailorService] Closed-loop refinement notice:', refineErr);
        }
      }

      const scoreGain = Math.max(0, verifiedScore - atsReport.overallScore);
      ...
      return {
        ...
        archetype,
        baselineAtsScore: atsReport.overallScore,
        verifiedAtsScore: verifiedScore,
        scoreGain,
        verificationReport: verifiedReport,
        iterationCount,
        loopStatus: verifiedScore >= targetScore ? 'converged_target' : 'single_pass_optimal',
      };
    } catch (err: any) {
  ...
  private async routeAndExecute(
    ...
    // Fast-path return if API key is not configured (unless local Ollama)
    if (!settings.apiKey?.trim() && provider !== 'ollama') {
      return { diffs: [], usedAi: false, error: 'No API key configured for provider' };
    }
...
Respond strictly with valid JSON conforming to the schema.`;
  ...
  private buildRefinementPrompt(
    currentDiffs: TailoredBulletDiff[],
    jobDescription: string,
    ...
    const systemPrompt = `You are Hacky, an elite AI career advisor in Closed-Loop Evaluator-Optimizer mode.
Your initial draft scored ${currentScore}/100. Your goal is to reach a top-tier ATS score of ${targetScore}+ by weaving the remaining critical technical keywords into the bullets.
Maintain strict line-budget compliance (no line wrapping) and enforce the Google XYZ formula: "Accomplished [X] as measured by [Y] by doing [Z]".
${strictAntiHallucination ? 'DO NOT invent unverified numbers; use bracketed placeholders like [X%] or [Y+] if needed.' : ''}
Output valid JSON adhering strictly to the schema.`;

    const bulletList = currentDiffs.slice(0, 3).map((d, i) => {
      const maxChars = Math.max(d.originalText.length, 160);
      return `[Bullet ${i + 1} | ID: ${d.id} | Max Budget: ${maxChars} chars]\nCurrent Version: "${d.tailoredText}"\nOriginal: "${d.originalText}"`;
    }).join('\n\n');

    const userPrompt = `TARGET: ${jobTitle} at ${company}
CURRENT EVALUATION: Score ${currentScore}/100. Target is ${targetScore}+.
CRITICAL MISSING SKILLS TO INJECT NATURALLY:
${stillMissingKeywords.map(k => `- ${k}`).join('\n')}

REFINE THESE BULLETS TO INJECT THE MISSING SKILLS WITHOUT INCREASING LINE COUNT:
${bulletList}
