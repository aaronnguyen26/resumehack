...
describe('Hacky AI Section Height Containment & Independent Scroll Engine', () => {
  ...
  it('verifies InAppDocumentCanvas bounds the workspace and aligns Hacky AI to the exact same length as the canvas', () => {
    ...
    // Root container must have overflow-hidden and min-h-0
    expect(content).toContain('flex flex-col h-full w-full bg-zinc-50 dark:bg-[#09090B]');
    ...
    // Main workspace viewport row must have items-stretch, overflow-hidden, min-h-0, and h-full
    expect(content).toContain('flex-1 flex flex-col lg:flex-row items-stretch justify-start relative overflow-hidden min-h-0 h-full');

    // Left canvas column must be h-full, min-h-0, and overflow-hidden
    expect(content).toContain('flex-1 flex flex-col min-w-0 bg-zinc-100/70 dark:bg-[#0c0c0e] relative h-full min-h-0 overflow-hidden');

    // Left paper stage must have flex-1, overflow-y-auto, and min-h-0 (no min-h-[850px] layout blowout)
    expect(content).toContain('flex-1 overflow-y-auto min-h-0 p-3 sm:p-5 md:p-6 lg:p-8 flex flex-col items-center justify-start relative');
    expect(content).not.toContain('min-h-[850px]');

    // Right Hacky AI aside wrapper must share items-stretch h-full with the left column
    expect(content).toMatch(/<aside\s+aria-label="Hacky AI ATS Inspector"\s+className="[^"]*h-80 sm:h-96 lg:h-full shrink-0 border-t lg:border-t-0 lg:border-l[^"]*min-h-0 overflow-hidden/);
  });

  it('verifies HackyAiAtsPanel implements independent internal scrolling without stretching its container', () => {
    const content = fs.readFileSync(panelPath, 'utf8');

    // Root aside has h-full, min-h-0, overflow-hidden
    expect(content).toMatch(/<aside\s+className="[^"]*w-full h-full[^"]*min-h-0 overflow-hidden/);
    ...
    expect(content).toMatch(/<div\s+className="flex-1 overflow-y-auto min-h-0 p-4 space-y-4">/);
  });
  ...
  it('verifies App.tsx locks canvas tab within viewport to prevent page lengthening', () => {
    ...
    expect(content).toContain("activeTab === 'canvas' ? 'max-w-[1780px] px-1 sm:px-2 md:px-3 py-1 h-[calc(100vh-64px)] min-h-0 overflow-hidden flex flex-col'");

    // Canvas container in App.tsx takes full height and flex-1 with min-h-0 overflow-hidden
    expect(content).toContain('<div className="w-full h-full flex-1 flex flex-col min-h-0 overflow-hidden animate-in fade-in duration-200">');
  });
